...
def _ensure_user(graph, uid):
    if uid not in graph.nodes:
        ...
        graph.add_node_without_validation(Node(id=uid, kinds=["User", "Base"], properties=p))
...
def _ensure_group(graph, gid):
    if gid not in graph.nodes:
        ...
        graph.add_node_without_validation(Node(id=gid, kinds=["Group", "Base"], properties=p))
...
def build_graph(groups, users, surrogat_profiles, unixpriv_profiles, facility_profiles, dataset_profiles, tcicstrn_profiles, gcicstrn_profiles):
    ...
    for g in groups:
        ...
        graph.add_node_without_validation(Node(id=g["name"], kinds=["Group", "Base"], properties=p))
    ...
    for u in users:
        ...
        node = Node(id=u["name"], kinds=["User", "Base"], properties=p)
    ...
    for g in groups:
        for u in g["users"]:
            ...
            graph.add_edge_without_validation(
                Edge(start_node=u["name"], end_node=g["name"], kind="MemberOf", properties=ep)
            )
    ...
    for g in groups:
        for sg in g["subgroups"]:
            ...
            graph.add_edge_without_validation(
                Edge(start_node=sg, end_node=g["name"], kind="SubGroupOf")
            )
